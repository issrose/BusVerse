...
async function loadInitialData() {
  try {
    let [stopsRes, routesRes] = await Promise.all([
      fetch('/api/stops').then(r => r.ok ? r.json() : null).catch(() => null),
      fetch('/api/routes').then(r => r.ok ? r.json() : null).catch(() => null)
    ]);

    if (!stopsRes || !stopsRes.length) {
      // Fallback for static hosting like GitHub Pages
      const staticDb = await fetch('busverse_db.json').then(r => r.json()).catch(() => null);
      if (staticDb) {
        stopsRes = staticDb.stops || [];
        routesRes = staticDb.routes || [];
        window._STATIC_DB = staticDb;
      }
    }
...
async function searchBuses() {
  ...
  try {
    ...
    if (!data) {
      if (!window._STATIC_DB) {
        window._STATIC_DB = await fetch('busverse_db.json').then(r => r.json()).catch(() => null);
      }
      if (window._STATIC_DB && window._STATIC_DB.buses) {
        const s = (state.source || '').toLowerCase().trim();
        const d = (state.destination || '').toLowerCase().trim();
        data = window._STATIC_DB.buses.filter(b => {
          const matchSrc = !s || (b.source && b.source.toLowerCase().includes(s));
          const matchDest = !d || (b.destination && b.destination.toLowerCase().includes(d));
          return matchSrc && matchDest;
        });
        if (!data.length) {
          data = window._STATIC_DB.buses.slice(0, 12);
        }
      }
    }
    ...
    listEl.innerHTML = `
      <div class="empty-state">
        ...
        <p>Could not load schedule data. Please refresh or verify network.</p>
      </div>
    `;
  }
}
